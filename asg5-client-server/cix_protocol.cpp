...
// Loads a file into a buffer and returns the buffer
string load_file (cix_header& header) {
   ...
   if(is) {
      ...
      buffer[length] = '\0';////////////////////////////// help @ problem?
      //if (is) cout << "all characters read successfully.";
      //else cout << "error: only " << is.gcount() << " could be read";
      is.close();
      string buf(buffer);
      return buf;
   }else {
